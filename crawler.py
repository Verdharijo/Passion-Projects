...
#function to crawl via making HTML document of systematic review
...
#function to loop process of crawling and print extracted_info.txt file
...
def get_pub3_with_pub2(inFile):
    f = open(inFile, 'r') #open file in read mode instead of binary mode
    reader = csv.reader(f)
    cntRow = -1
    ...
    l_doi = list(d_doi_pmids.keys())

    # for i in l_doi:
    # use range instead of xrange as it is deprecated
    for i in range(len(l_doi)): 
        doi = l_doi[i]
...
if __name__ == '__main__':
    ...
    while yourFolder.strip() == '':
        yourFolder = input("> Enter your folder name: ")
