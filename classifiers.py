# -*- coding: utf-8 -*-
from __future__ import division # Ensures compatibility with future versions of Python
import sys
import os
...
# LogisticRegression Fix
# Error importing older models trained in older sklearn==0.20 and now using sklearn==1.0
# Troubleshot found on https://www.kaggle.com/code/adeepak7/load-old-sklearn-models-with-new-sklearn-package
import sys
...
def ci_bootstrap_calibrationcurve(clf, X, y, numbsampling=2000):
    ...
    ltup_lower_upper_x = []
    ltup_lower_upper_y = []
    for i in range(len(d_group_d_xy_l)):
...
def calibration(X_test, y_test, l_clf_modelname):
    ...
    for name in d_name_l_mean_predicted_value_x:
        ...
        for ix in range(len(l_mean_predicted_value_x)):
...
def ci_bootstrap_train(clf_name, optimised_clf, optimised_clf_param, X, y, features_name,
    ...
    for _ in range(n):
        ...
        if clf_name == 'Logistic regression':
            ...
            for i in range(len(featimportance)):
...
def load_var(inFile):
    ...
    with open(inFile + '.cpickle', 'rb') as f:
